...
class MCPHandler {
  ...
  buildIntents() {
    return [
      { name: 'ORDER_DISH', patterns: ['来个', '我要', '给我来', '点一份', '再加一份', '再来一个', '份', '打包一份', '外带一份'], keywords: ['份', '个', '盘', '碗', '打包', '外带'], confidence: 0.9 },
      { name: 'REMOVE_FROM_CART', patterns: ['不要了', '退掉', '取消', '去掉', '不要这个', '移除', '删掉'], keywords: ['不要', '取消', '退', '移除', '删'], confidence: 0.85 },
      { name: 'VIEW_CART', patterns: ['看看购物车', '我的订单', '点了什么', '查看订单', '购物车', '当前订单'], keywords: ['购物车', '订单', '查看'], confidence: 0.9 },
      { name: 'MODIFY_ORDER', patterns: ['不要香菜', '少辣', '加辣', '微辣', '不要葱', '多放', '少放', '改一下', '调整'], keywords: ['不要', '少', '多', '加', '改', '调整', '辣', '香菜', '葱'], confidence: 0.8 },
      { name: 'CONFIRM_ORDER', patterns: ['好的', '可以', '就这样', '下单', '提交', '确认', '要了', '行', '完成了', '好了'], keywords: ['下单', '提交', '确认', '要', '完成', '好'], confidence: 0.9 },
      { name: 'CANCEL_ORDER', patterns: ['取消订单', '退单', '不点了', '不要订单', '撤销订单'], keywords: ['取消', '退', '撤销', '不点'], confidence: 0.85 },
      { name: 'QUERY_MENU', patterns: ['有什么', '推荐', '菜单', '招牌', '有什么好吃的', '菜品有哪些', '都有什么菜'], keywords: ['菜单', '推荐', '招牌', '有什么', '菜品'], confidence: 0.85 },
      { name: 'QUERY_PRICE', patterns: ['多少钱', '价格', '怎么卖', '贵不贵', '报价', '价位'], keywords: ['钱', '价格', '贵', '便宜', '价位'], confidence: 0.9 },
      { name: 'QUERY_SPECIFIC_DISH', patterns: ['宫保鸡丁', '鱼香肉丝', '红烧肉', '糖醋里脊', '麻婆豆腐'], keywords: [], confidence: 0.95, isDishName: true },
      { name: 'REPEAT_ORDER', patterns: ['跟上次一样', '和上次一样', '老样子', '还点那个', '再来一份', '再点一次'], keywords: ['上次', '一样', '老样子', '再来'], confidence: 0.9 },
      { name: 'RESERVE_TABLE', patterns: ['预约', '订座', '订位', '预订', '订包间', '订包厢', '提前订'], keywords: ['预约', '订座', '订位', '预订', '包间', '包厢'], confidence: 0.9 },
      { name: 'RESERVATION_INFO', patterns: ['几位', '多少人', '几个人', '座位', '位置', '靠窗', '安静'], keywords: ['位', '人', '座位', '位置', '靠窗', '安静'], confidence: 0.8 },
      { name: 'QUERY_DELIVERY', patterns: ['外卖', '送餐', '可以送吗', '能送外卖吗', '配送', '送到家'], keywords: ['外卖', '送', '配送', '到家'], confidence: 0.85 },
      { name: 'DELIVERY_TIME', patterns: ['多久送到', '配送时间', '多长时间', '什么时候到', '等多久'], keywords: ['多久', '时间', '等', '到'], confidence: 0.85 },
      { name: 'PAYMENT_METHOD', patterns: ['怎么付款', '支付方式', '能用什么付', '支持什么支付', '微信', '支付宝', '现金'], keywords: ['支付', '微信', '支付宝', '现金', '付款'], confidence: 0.85 },
      { name: 'APPLY_COUPON', patterns: ['优惠券', '用券', '打折', '优惠码', '促销码', '满减'], keywords: ['券', '优惠', '打折', '满减', '折扣'], confidence: 0.85 },
      { name: 'SPLIT_BILL', patterns: ['分单', '分开付', 'AA', '各付各的', '分开结账'], keywords: ['分', 'AA', '各', '开'], confidence: 0.85 },
      { name: 'QUERY_POINT', patterns: ['积分', '多少积分', '积分查询', '积分怎么用'], keywords: ['积分'], confidence: 0.9 },
      { name: 'RECHARGE', patterns: ['充值', '充钱', '充会员', '续费'], keywords: ['充值', '充'], confidence: 0.9 },
      { name: 'BIRTHDAY_VIP', patterns: ['生日', '寿星', '过生日', '生日优惠', '生日礼物'], keywords: ['生日', '寿星'], confidence: 0.9 },
      { name: 'MEMBER_REGISTER', patterns: ['成为会员', '注册会员', '加入会员', '新会员'], keywords: ['会员', '注册', '加入'], confidence: 0.85 },
      { name: 'ASK_FAQ', patterns: ['几点', '怎么', '什么', '能不能', '可以', '有没有', '地址', '电话'], keywords: ['怎么', '什么', '为什么', '包间', '打包', '地址', '电话', '营业'], confidence: 0.8, useFAQ: true },
      { name: 'WIFI_QUERY', patterns: ['WiFi', '无线网', 'wifi密码', '上网', '无线网络'], keywords: ['WiFi', 'wifi', '无线', '上网'], confidence: 0.95, useFAQ: true },
      { name: 'PARKING_QUERY', patterns: ['停车', '停车场', '停车位', '停车费'], keywords: ['停车', '车场', '车位'], confidence: 0.95, useFAQ: true },
      { name: 'BUSINESS_HOURS', patterns: ['几点开门', '几点营业', '营业时间', '什么时候开门', '几点关门', '营业到几点'], keywords: ['开门', '营业', '关门', '时间'], confidence: 0.95, useFAQ: true },
      { name: 'FEEDBACK', patterns: ['投诉', '意见', '反馈', '不好吃', '上菜慢', '退款', '差评'], keywords: ['投诉', '意见', '反馈', '退款', '差评', '不好'], confidence: 0.85 },
      { name: 'COMPLIMENT', patterns: ['好吃', '表扬', '夸一下', '写好评', '赞', '不错'], keywords: ['好吃', '表扬', '夸', '赞', '不错', '好'], confidence: 0.85 },
      { name: 'ALLERGEN_QUERY', patterns: ['过敏', '食物过敏', '忌口', '不能吃', '不要放'], keywords: ['过敏', '忌口', '不能'], confidence: 0.9 },
      { name: 'SPICY_LEVEL', patterns: ['微辣', '中辣', '特辣', '不辣', '少辣', '要辣', '不要辣'], keywords: ['辣'], confidence: 0.9 },
      { name: 'TASTE_PREFERENCE', patterns: ['清淡', '少油', '少盐', '甜', '咸', '酸'], keywords: ['清淡', '油', '盐', '甜', '咸', '酸'], confidence: 0.85 },
      { name: 'GREETING', patterns: ['你好', '您好', 'hi', 'hello', '嗨', '在吗', '有人吗', '在不在'], keywords: ['你好', '您好', 'hi', 'hello', '嗨', '在', '有人'], confidence: 0.9 },
      { name: 'GOODBYE', patterns: ['再见', '拜拜', '走了', '谢谢', '感谢', '辛苦了'], keywords: ['再见', '拜拜', '走', '谢谢', '辛苦'], confidence: 0.9 },
      { name: 'TABLE_NUMBER', patterns: ['桌号', '桌位', '座位号', '几桌'], keywords: ['桌', '位号'], confidence: 0.8 }
    ];
  }

  buildEntityPatterns() {
    return {
      dishName: /宫保鸡丁|鱼香肉丝|红烧肉|糖醋里脊|麻婆豆腐|水煮肉片|回锅肉|酸菜鱼|剁椒鱼头|糖醋排骨|红烧狮子头|东坡肉|狮子头|京酱肉丝|木须肉|四季豆|土豆丝|酸辣土豆丝|炒时蔬|蚝油生菜|干煸四季豆|蒜蓉西兰花|香菇油菜|番茄炒蛋|蛋炒饭|扬州炒饭|西红柿鸡蛋面|牛肉面|刀削面|炸酱面|担担面|小笼包|蒸饺|煎饺|锅贴|豆浆|油条|豆腐脑|紫菜蛋花汤|酸辣汤|西红柿鸡蛋汤|疙瘩汤|米饭|馒头|花卷|烙饼/g,
      number: /\d+/,
      spiceLevel: /微辣|中辣|特辣|不辣|少辣/g,
      dietaryRestriction: /不要香菜|不要葱|不要蒜|不要辣|不放香菜|不放葱|不放蒜|少油|少盐|少糖|清淡/g,
      timeExpression: /现在|马上|立刻|等一下|稍等|一会儿|十分钟|二十分钟|半小时|一个小时/g
    };
  }
  ...
  extractEntities(message) {
    const entities = { dishes: [], numbers: [], spiceLevel: null, dietaryRestrictions: [], timeExpression: null };
  ...
  needsHumanTransfer(sessionId, intent, message) {
    ...
    const complexPatterns = ['投诉', '退款', '开发票', '吵架', '生气', '要投诉', '叫你们老板', '经理', '赔偿', '报警', '曝光', '媒体', '法律', '律师', '起诉'];
  ...
  async handleMessage(sessionId, customerId, message, inputType = 'text') {
    try {
      ...
      if (useFAQ || intent === 'ASK_FAQ' || intent === 'WIFI_QUERY' || intent === 'PARKING_QUERY' || intent === 'BUSINESS_HOURS') {
      ...
      if (result.type === 'text' && !result.excludeRecommendation && intent !== 'GREETING' && intent !== 'GOODBYE' && intent !== 'FEEDBACK') {
      ...
      return { type: 'error', reply: '抱歉，系统有点问题，请稍后再试！' };
    }
  }

  updateSessionState(sessionId, intent) {
    const stateMap = {
      'ORDER_DISH': DialogState.ORDERING, 'QUERY_SPECIFIC_DISH': DialogState.ORDERING,
      'REMOVE_FROM_CART': DialogState.MODIFYING, 'MODIFY_ORDER': DialogState.MODIFYING,
      'CONFIRM_ORDER': DialogState.CONFIRMING, 'CANCEL_ORDER': DialogState.ORDERING,
      'QUERY_MENU': DialogState.ORDERING, 'RESERVE_TABLE': DialogState.RESERVATION,
      'FEEDBACK': DialogState.FEEDBACK, 'GREETING': DialogState.GREETING,
  ...
  async handleOrderDish(sessionId, message, entities) {
    ...
    return { type: 'text', reply: '抱歉，我没找到您说的菜品，您可以告诉我完整菜名，或者说"推荐"我帮您推荐~' };
  }
  ...
  async handleRemoveFromCart(sessionId, message) {
    ...
    for (const item of session.currentCart) {
      if (message.includes(item.name)) {
        ...
        return { type: 'cart_updated', reply: `好的，已移除「${item.name}」`, cart: this.context.getOrCreateSession(sessionId).currentCart };
      }
    }

    const lastItem = session.currentCart[session.currentCart.length - 1];
    this.context.removeFromCart(sessionId, lastItem.dishId);
    return { type: 'cart_updated', reply: `好的，已移除最后一个「${lastItem.name}」`, cart: this.context.getOrCreateSession(sessionId).currentCart };
  }
  ...
  async handleViewCart(sessionId) {
    ...
    return { type: 'cart_view', reply: `您的购物车：\n\n${itemsText}\n\n总计：¥${total}\n\n确认下单吗？`, cart: session.currentCart, total };
  }

  async handleModifyOrder(sessionId, customerId, message, entities) {
    const lowerMsg = message.toLowerCase();

    if (lowerMsg.includes('香菜')) this.context.addDislike(customerId, '香菜');
    if (lowerMsg.includes('葱')) this.context.addDislike(customerId, '葱');
    if (lowerMsg.includes('蒜')) this.context.addDislike(customerId, '蒜');
    ...
    if (entities.spiceLevel) {
      ...
      if (lowerMsg.includes('微辣')) this.context.addTastePreference(customerId, '微辣');
      else if (lowerMsg.includes('中辣')) this.context.addTastePreference(customerId, '中辣');
      else if (lowerMsg.includes('特辣')) this.context.addTastePreference(customerId, '特辣');
    }
    ...
    return { type: 'text', reply: '好的，已记录您的口味要求~' };
  }
  ...
  async handleConfirmOrder(sessionId, customerId) {
    ...
    const syncResult = await this.tools.syncOrder({ orderNo, items: session.currentCart, total });

    if (syncResult.success) {
      this.context.recordOrder(customerId, { orderNo, items: session.currentCart, total });
      this.context.clearCart(sessionId);
      return { type: 'order_confirmed', reply: `✅ 下单成功！\n\n订单号：${orderNo}\n金额：¥${total}\n\n感谢惠顾，请稍候~`, orderNo, total };
    }

    return { type: 'error', reply: '下单失败了，稍后再试可以吗？' };
  }
  ...
  async handleCancelOrder(sessionId) {
    ...
    this.context.clearCart(sessionId);
    return { type: 'text', reply: '好的，已为您取消订单。欢迎下次光临~' };
  }
  ...
  async handleQueryMenu(sessionId, context) {
    if (!this.recommendationEngine) {
      ...
      return { type: 'menu', reply: `我们的菜品：\n\n${menuText}\n\n想点什么？`, dishes: result.dishes };
    }
    ...
    if (recommendations.popular?.length > 0) {
      reply += '🔥 今日爆款：\n';
      recommendations.popular.slice(0, 3).forEach(dish => { reply += `  • ${dish.name} - ¥${dish.price}\n`; });
    }
    if (recommendations.combo?.length > 0) {
      reply += '\n🍱 超值套餐：\n';
      recommendations.combo.slice(0, 2).forEach(combo => { reply += `  • ${combo.name} - ¥${combo.comboPrice}（省${combo.discount}元）\n`; });
    }

    return { type: 'menu', reply };
  }
  ...
  async handleQueryPrice(sessionId, message, entities) {
    if (entities.dishes.length > 0) {
      ...
      if (dish) {
        return { type: 'text', reply: `「${dish.name}」的价格是 ¥${dish.price}` };
      }
    }
    
    return { type: 'text', reply: '请问您想查询哪个菜品的价格？' };
  }
  ...
  async handleRepeatOrder(sessionId, customerId) {
    ...
    return { type: 'cart_updated', reply: `已复刻您上次的订单：\n\n${lastOrder.items.map(i => `${i.name} x${i.quantity || 1}`).join('\n')}\n\n总计：¥${total}\n\n确认下单吗？`, cart: this.context.getOrCreateSession(sessionId).currentCart };
  }

  async handleReserveTable(sessionId, message, entities) {
    const peopleCount = entities.numbers.length > 0 ? entities.numbers[0] : null;
    return { type: 'reservation', reply: peopleCount ? `好的，帮您预约${peopleCount}位！请问您想预约什么时间？` : '请问您几位用餐？我来帮您安排~' };
  }

  async handleQueryDelivery(sessionId) {
    return { type: 'text', reply: '我们支持外卖配送！一般情况下，制作完成后30分钟左右送达。请问您想点些什么？' };
  }

  async handlePaymentMethod(sessionId) {
    return { type: 'text', reply: '我们支持以下支付方式：\n• 微信支付\n• 支付宝\n• 现金\n• 会员卡余额\n\n请问您想用哪种方式支付？' };
  }

  async handleApplyCoupon(sessionId, message) {
    const couponMatch = message.match(/[A-Z0-9]{4,}/i);
    if (couponMatch) {
      return { type: 'text', reply: `正在验证优惠码 ${couponMatch[0]}...` };
    }
    return { type: 'text', reply: '请告诉我您的优惠码~' };
  }

  async handleSplitBill(sessionId) {
    return { type: 'text', reply: '好的，支持分开结账！请问您想怎么分？' };
  }

  async handleQueryPoint(customerId) {
    const profile = this.context.getCustomerProfile(customerId);
    return { type: 'text', reply: `您当前有 ${profile.totalSpent || 0} 积分，100积分可抵扣1元现金~` };
  }

  async handleRecharge(sessionId) {
    return { type: 'text', reply: '会员充值有优惠！\n• 充值200送20\n• 充值500送80\n• 充值1000送200\n\n请问您想充多少？' };
  }

  async handleBirthdayVip(customerId) {
    return { type: 'text', reply: '🎂 会员生日当天享受菜品8折优惠（酒水除外），还送长寿面一份！请问您是会员吗？' };
  }

  async handleMemberRegister(sessionId) {
    return { type: 'text', reply: '成为会员可以享受积分返利、会员折扣、生日优惠等多重权益！请问您的手机号是多少？' };
  }

  async handleFeedback(sessionId, message) {
    return { type: 'feedback', reply: '非常抱歉给您带来不好的体验！请您告诉我具体情况，我会及时反馈给店长处理~' };
  }

  async handleCompliment(sessionId) {
    return { type: 'text', reply: '谢谢您的认可！您的支持是我们最大的动力~请问还有什么需要吗？' };
  }
  ...
  async handleGreeting(sessionId, customerId) {
    ...
    if (isReturning) {
      return { type: 'greeting', reply: `😊 欢迎回来！${profile.preferences?.tastes?.join('、') || ''}对吧？\n\n想吃点什么呢？`, isReturning };
    }
    
    return { type: 'greeting', reply: '👋 您好！欢迎光临！\n\n我是您的AI点餐助手，有什么可以帮您？', isReturning: false };
  }

  async handleGoodbye(sessionId) {
    this.context.clearCart(sessionId);
    return { type: 'goodbye', reply: '感谢光临！祝您用餐愉快~欢迎下次再来！👋' };
  }
  ...
  async handleTableNumber(sessionId, message) {
    ...
    if (tableMatch) {
      ...
      return { type: 'text', reply: `好的，您是${tableNum}号桌~` };
    }
    return { type: 'text', reply: '请问您的桌号是多少？' };
  }
  ...
  async handleWelcome(sessionId, customerId) {
    ...
    return { type: 'welcome', reply: welcome + '想吃点什么？我可以帮您推荐菜品，或者直接告诉我菜名~', isReturning };
  }
}
